def foldX(paper, x, maxY):
    ...
    for i in range(0, x):
        for yIndex in range(0, maxY): 
            if(paper[yIndex][paperSize] == '#'): paper[yIndex][i] = paper[yIndex][paperSize]
...
def part1(inputPaper, folds, paperMax):
    ...
    for fold in folds:
        if(fold[0] == 'y'): 
            foldY(paper, fold[1], len(paper[0]) - 2)
        else: 
            foldX(paper, fold[1], len(paper) - 1)
        for i in paper:
            print(' '.join(i))
            hashtags +=  ' '.join(i).count('#')
    print(hashtags)
...
def main():
    ...
    for i in range(len(input)): 
        ...
        x, y = inputPaper[i][0], inputPaper[i][1]
        if(x > paperMax[0]): paperMax = (x, paperMax[1])
        if(y> paperMax[1]): paperMax = (paperMax[0], y)
